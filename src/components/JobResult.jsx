...
function JobResult({ data, favourites, addToFavourites, removeFromFavourites }) {
  ...
  const isFav = fav.includes(data.company_name)
  // console.log(isFav, favourites)
  const toggleFavourite = () => {
    isFav ? dispatch(removeFromFav(data.company_name)) : dispatch(addToFav(data.company_name))
  }

  return (
    <Row className="mx-0 mt-3 p-3" style={{ border: "1px solid #00000033", borderRadius: 4 }}>
      <Col xs={3} className="d-flex">
        {isFav ? <StarFill color="gold" size={16} className="me-4 my-auto" onClick={toggleFavourite} /> : <Star color="gold" size={16} className="me-4 my-auto" onClick={toggleFavourite} />}
